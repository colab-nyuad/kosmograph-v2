...
//k-hop neighbpours exploration
  const getKHopNeighbors = (startNode: string, k: number) => {
    ...
    for (let i = 0; i < k; i++) {
      ...
      for (const node of frontier) {
        globalGraph.forEachNeighbor(node, (neighbor) => {
      ...
      frontier.clear();
      nextFrontier.forEach(node => frontier.add(node));
    }
  ...
  const onCosmographClick = React.useCallback(
    (n) => {
      if (n) {
        ...
        if (isKHopEnabled) {
          const kHopNodes = getKHopNeighbors(n.id, kHopValue);
          neighbors = kHopNodes.map(nodeId => ({
            id: nodeId,
            indegree: 0,
            outdegree: 0
          }));
    ...
    [globalGraph, kHopValue, isKHopEnabled]
  );
